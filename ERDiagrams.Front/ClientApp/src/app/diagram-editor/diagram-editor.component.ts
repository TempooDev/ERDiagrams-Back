...
//@ts-ignore
  ...
  public diagramModelChange = function(changes: go.IncrementalData) {
    ...
    this.state = produce(this.state, draft => {
      ...
      draft.diagramNodeData = DataSyncService.syncNodeData(changes, draft.diagramNodeData, diaComp.observedDiagram.model);
      draft.diagramLinkData = DataSyncService.syncLinkData(changes, draft.diagramLinkData, diaComp.observedDiagram.model);
      ...
      if (modifiedNodeDatas && draft.selectedNodeData) {
        for (let i = 0; i < modifiedNodeDatas.length; i++) {
          ...
          const nodeKeyProperty = diaComp.myDiagramComponent.diagram.model.nodeKeyProperty as string;
  ...
  public ngAfterViewInit() {
    ...
    const diaComp: DiagramEditorComponent = this;
    // listener for inspector
    this.myDiagramComponent.diagram.addDiagramListener('ChangedSelection', function(e) {
      if (e.diagram.selection.count === 0) {
        //@ts-ignore
        diaComp.selectedNodeData = null;
      }
      const node = e.diagram.selection.first();
      //@ts-ignore
      diaComp.state = produce(appComp.state, draft => {
